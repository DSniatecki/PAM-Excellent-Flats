...
const FlatAdScreen = ({ flat, updateFlat }) => {
    const { isFavourite, title, price, media, details, description, contact, location } = flat;
    return (
      <SafeAreaView style={styles.container}>
        <AppHeader screenTitle={"Ogłoszenie"} isFavourite={isFavourite} changeIsFavourite={() => {
    ...
    return (
      <>
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.boldText}>{`${numberOfRooms} ${numberOfRooms > 1 ? "pokoje" : "pokój"}`}</Text>
        </View>
        <View style={styles.row}>
